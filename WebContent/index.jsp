	...
	<div id="middle">
			...
			<div class="pre_look">
				...
				<dl>
					<c:forEach items="${historyMap}" var="temp" end="3">
						...
						<dd>
							<a href="doAction?action=pview&pId=${temp.value.hp_Id }">${temp.value.hp_Name }</a>
						</dd>
					</c:forEach>
		...
		<div class="p_center">
			...
			<ul class="product2">
				...
				<c:forEach items="${products}" var="p">
					<li>
						<dl>
							<dt>
								<a href="doAction?action=pview&pId=${p.hp_Id }" target="_self"><img
									src="${p.hp_file_name }" /></a>
							</dt>
							<dd class="title">
								<a href="doAction?action=pview&pId=${p.hp_Id}" target="_self">${p.hp_Name }</a>
							</dd>
			...
			<div class="pager">
				<ul>
					<ul>
						<li><a href="doAction?action=page&page=${pager.prevPage}&cate=${param.cate}&hpcId=${param.hpcId}&qname=${requestScope.qname}" id="nextPage" id="lastPage">上一页</a></li>
						<c:forEach items="${requestScope.pager.groupList}" var="pl">
							...
							<c:if test="${pl!=pager.currentPage }">
								<li><a href="doAction?action=page&page=${pl}&cate=${param.cate}&hpcId=${param.hpcId}">${pl}</a></li> 
							</c:if>
						</c:forEach>
						<li><a href="doAction?action=page&page=${pager.nextPage}&cate=${param.cate}&hpcId=${param.hpcId}&qname=${requestScope.qname}" id="nextPage">下一页</a></li>
					</ul>
				</ul>
				<input type="hidden" id="source" 		value="${requestScope.pageModel.source}" />
				<input type="hidden" id="hpcId" 		value="${requestScope.pageModel.hpcId}" /> 
